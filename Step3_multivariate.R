...
targets
group <- targets$group #assigning column as character vector
group <- factor(group) #telling it there are 2 categories, "levels"
...
plot(clusters, labels=sampleLabels) #HS05 is an outlier, if you look at fastqc, you'll kind of see it

# Principal component analysis (PCA) -------------
pca.res <- prcomp(t(log2.cpm.filtered.norm), scale.=F, retx=T)
#look at the PCA result (pca.res) that you just created
ls(pca.res)
summary(pca.res) # Prints variance summary for all principal components. 1 PC for each sample
...
ggplot(pca.res.df) +
  ...
  #geom_label() +
  stat_ellipse() +
  xlab(paste0("PC1 (",pc.per[1],"%",")")) + #also pull from things we created previously, so we want to add in %
  ylab(paste0("PC2 (",pc.per[2],"%",")")) + #things to be pasted are separated by ""
  labs(title="PCA plot",
       caption=paste0("produced on ", Sys.time())) +
coord_fixed()+ #to fix axes so that PC1 has bigger axis
  theme_bw()
...
ggplot(pca.pivot) +
  ...
  facet_wrap(~PC) + #actual small multiples
  labs(title="PCA 'small multiples' plot",
       caption=paste0("produced on ", Sys.time())) +
  theme_bw() +
  coord_flip()
#for PC4 is 2 healthy and 2 disease from same batch? could be batch effect
...
mydata.df <- log2.cpm.filtered.norm.df %>% 
  mutate(healthy.AVG = (HS01 + HS02 + HS03 + HS04 + HS05)/5, #add new columns as a function of existing data
         disease.AVG = (CL08 + CL10 + CL11 + CL12 + CL13)/5,
         #now make columns comparing each of the averages above that you're interested in
         LogFC = (disease.AVG - healthy.AVG)) %>% 
  mutate_if(is.numeric, round, 2) #mutate only under certain conditions, round to 2 decimal places
...
mydata.sort <- mydata.df %>%
  dplyr::arrange(desc(LogFC)) %>% #descending order
...
mydata.filter <- mydata.df %>% #select rows
  ...
  dplyr::select(geneID, healthy.AVG, disease.AVG, LogFC) %>% #| is or
...
pca.res <- prcomp(t(log2.cpm.filtered.norm), scale.=F, retx=T)
pc.var <- pca.res$sdev^2 # sdev^2 captures these sign values from the PCA result
